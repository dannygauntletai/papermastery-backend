"""
Test script to verify the Gemini PDF processing functionality.

This script:
1. Submits a paper for processing
2. Waits for the paper to be processed
3. Sends a chat message to the paper
4. Verifies that a response is received with sources
"""
...
TEST_USER_ID = os.getenv("TEST_USER_ID")  # Replace with a valid user ID
...
async def main():
    ...
    # Send a chat message
    query = "What are the main findings of this paper? Please include quotes and references."
    ...
    if chat_response.get('sources'):
        print("\nSources:")
        for i, source in enumerate(chat_response.get('sources')):
            print(f"Source {i+1}:")
            print(f"  Text: {source.get('text')}")
            print(f"  Reference: {source.get('metadata', {}).get('reference', 'N/A')}")
    ...
    if not chat_response.get('sources'):
        print("❌ Test failed: No sources included")
        return
    
    print("\n✅ Test passed: Received non-empty response with sources")
